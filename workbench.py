...
class fenster:
	def __init__(self, typ, fenstername, fensterrect):
		...
		self.outfensterrect = (self.fensterrect.x-4,self.fensterrect.y-22,self.fensterrect.w+21,self.fensterrect.h+25)
		self.fenstername = fenstername
		self.hinweistext = []
		self.icons = []
		self.ismausaktiv = False
	...
	def ausgabe(self, screen):
		...
		if self.ismausaktiv==True:			
			pygame.draw.rect(self.screen, (255, 0, 0), self.outfensterrect, 2)
		return
			...
			if leftclick:
				if pygame.Rect(afenster.closerect()).collidepoint(leftclickpos):
					...
					if afenster.fensterrect==afenster.savefensterrect:
						...
						print(afenster.fensterrect,afenster.savefensterrect)
...
#				running=False
				...
				aktivfenster.append(fenster("shell","AmigaShell",pygame.Rect(120,60,560,400)))
